...
export const outletController = {
  async list(req: NextRequest) {
    ...
    const result = await outletService.list(q);
  ...
  async getById(req: NextRequest, id: string) {
    requireAuth(req);
    return ok(await outletService.getDetail(id));
  },

  async create(req: NextRequest) {
    const session = requireRole(req, "sales");
    const input = await parseBody(req, outletCreateSchema);
    const c = await outletService.create(input, { id: session.sub, name: session.name });
    return created(c);
  },
  ...
  async update(req: NextRequest, id: string) {
    ...
    const u = await outletService.update(id, patch, { id: session.sub, name: session.name });
    return ok(u);
  },
  ...
  async performance(req: NextRequest) {
    ...
    return ok(await outletService.performanceSummary(url.get("area") ?? undefined));
  },
};
