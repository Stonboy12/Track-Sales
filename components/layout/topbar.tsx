...
const notifications = [
  { id: 1, title: "Komplain baru dari Toko Sumber Rejeki", time: "5 menit lalu", unread: true },
  { id: 2, title: "Target mingguan Anda 78% tercapai", time: "1 jam lalu", unread: true },
  { id: 3, title: "Harga kompetitor untuk Indomilk berubah", time: "3 jam lalu", unread: true },
  { id: 4, title: "Visit ke Indomaret Sudirman terjadwal besok", time: "Kemarin", unread: false },
];
...
export function Topbar({ role, userName }: TopbarProps) {
  ...
  const roleLabel = role === "admin" ? "Administrator" : role === "sales" ? "Sales" : "—";
    ...
    <header className="sticky top-0 z-40 flex h-16 items-center gap-2 border-b bg-background/80 px-4 backdrop-blur lg:px-6">
      <Sheet>
        ...
        <SheetContent side="left" className="w-64 p-0">
          <Sidebar className="border-r-0" role={role} />
        </SheetContent>
      </Sheet>

      <div className="relative hidden flex-1 max-w-md md:block">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input placeholder="Cari outlet, produk, atau salesman..." className="pl-9" />
      </div>

      <div className="flex flex-1 items-center justify-end gap-1.5 md:flex-none">
        <Button variant="ghost" size="icon" className="md:hidden" aria-label="Cari">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-80">
            ...
            <div className="max-h-80 overflow-y-auto">
              {notifications.map((n) => (
                <DropdownMenuItem key={n.id} className="flex items-start gap-2 px-3 py-2.5">
